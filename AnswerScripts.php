...
echo '<input type="text" id="pending-ans-scriptTB" class="transparent" value="Pending Answer-Scripts" readonly style="" />';
...
if ($result1->num_rows > 0)
{
	...
	for ($i = 0; $i < sizeof($HidIds); $i++)
	{
		...
		if ($result2->num_rows > 0)
		{
			...
			$tempPercent = "$HidIds[$i]    ({$percentrounded}%)";
			if($percent!=100){
				
				echo "<a class='HidIdAnchor' href='correctionpage.php?hidid={$HidIds[$i]}'> {$tempPercent} </a>";
			}
